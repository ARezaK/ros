...
1) Code depends on the robot having positive x and y coordinates on the map to make the math easier. Will possibly run with negative
   coordinates but program may act up or seg fault. (Latter is much more probable). You'll know your running in negative coords if the
    program says "Got Map, done init, then crashes".

2) Always set map_prob_left and map_prob_right array sizes to the size of the map.
   Variable spaces in similar parts of memory will be overwritten otherwise.

3) When running the program and it seg faults immediatly it means it didnt run setup (OR LOOK AT Precaution #1).
   Just restart the program. *Noted in the TODO

4) Currently extractLocalMap is subscribed to the odom topic even though I am using the transform.
   (I figured the transform would be better and its good practice to have an understanding of how to use the Tf topic in c++).
    The reason for this is that robot_x and robot_y are having their position set by the pose (I currently have that commented out)
    instead of my own transform due to thats how Leonardo is doing it and I am not sure if there is significance for that.
    If there is then you can leave it be otherwise it makes more sense to have that function subscribe to something else
    (ie. the transform topic) or just move it to a diff function.

5) Anything not explained here is explained in the paper or leonardos code.
*/
...
int localmapsize = 150;                          //Set this to the size you want local map to be.
...
void extractLocalMap(const nav_msgs::Odometry::ConstPtr& msg){
    ...
    should already be publishing the transform from the odom frame to the base_link frame.
     ...
     You might notice that local map takes everything from full map, including the wierd lane line stuff. As I understand,
     this is not a problem b/c the local map itself is never used but rather for visualzation purposes. Its the local map lidar scan
     that is used by other nodes
     */
    ...
    robotx = map_x - full_map.info.origin.position.x; //Look at precuation #4
    roboty = map_y - full_map.info.origin.position.y;

  
    int offset = localmapsize/2; //We used the offset to place us within the center of the local map
    ...
    int i = 0;
    //Two for loops create a square around the robots position. If you want to adjust for heading simply adjust the loops
    //to account for the heading. Heading can be retrieved from the variable above.
    for (int iy=iy2; iy<(iy2+localmapsize); iy++) {
        for (int ix=ix2; ix<(ix2+localmapsize); ix++) {
            localmap_occup1.data[i] = full_map.data[MAP_IDX(full_map.info.width, ix, iy)]; //Stick all the data in our
            //coordinates from our full map into the local map
...
int main(int argc, char **argv) {
    ...
    ros::NodeHandle n;
    //INIT the local map lidar scans
    //LOCALMAP lanelines
    ...
    ros::Publisher localmap1Pub = n.advertise<sensor_msgs::LaserScan>("localmap1", 1); //Local1 map (lanelines) Lidar scan
