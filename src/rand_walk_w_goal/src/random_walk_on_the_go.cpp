...
public:
    ...
    const static double MIN_SCAN_ANGLE_RAD = -15.0 / 180 * M_PI; //adjusted to provide more directionality
    ...
    const static double ROTATE_SPEED_RADPS = 1; //0.5 worked
    float closestRange;
...
void RandomWalk::processSensors() {
    ...
    while (ros::ok()) { // Keep spinning loop until user presses Ctrl+C
        ...
        //To launch roslaunch rand_walk_w_goal random_walk.launch
        //**********ON THE GO ALGO ********************************//
        move(0.5, rotateDir * ROTATE_SPEED_RADPS);//Start the robot moving forward and rotating

        //check if all all the data points provided by the narrowed lidar scan are greater than the closest range
        //If they are not continue executing the command above. However if they are execute the command
        //move with a forward velocity of 0.3. This structure of code has the benefit of continuously checking to
        //make sure it is going in a path that is "clear" of obstacles. On the other hand this method is slightly
        //imprecise and does not work at high turning speeds thus if the robot is placed too close to the fence when
        //starting out it will hit the fence and become stuck.
